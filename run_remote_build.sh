...
cd build && rm -rf ../build_output.log && DOCKER_HOST="$REMOTE_HOST" BUILDX_BAKE_ENTITLEMENTS_FS=0 BUILDKIT_STEP_LOG_MAX_SIZE=-1 docker buildx bake \
  --allow=fs=./.docker-cache/euro-office/desktop-js \
  --allow=fs=./.docker-cache/euro-office/web-apps \
  --allow=fs=./.docker-cache/euro-office/core-base \
  --allow=fs=./.docker-cache/euro-office/desktop-builder \
  --allow=fs.read=.. \
